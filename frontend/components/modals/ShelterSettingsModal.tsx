...
interface ShelterSettingsModalProps {
  visible: boolean;
  onClose: () => void;
  shelterData: ShelterData | null;
}

const ShelterSettingsModal: React.FC<ShelterSettingsModalProps> = ({
  visible,
  onClose,
  shelterData
}) => {
  ...
  const handleSupport = () => {
    ...
    const subject = 'Shelter Support Request';
    const body = `Hello Pawner Support,\n\nShelter: ${shelterData?.shelterName || 'Unknown'}\nEmail: ${shelterData?.user?.email || 'Unknown'}\n\nI need help with:\n\n`;
  ...
  const handleSectionToggle = useCallback((settingsId: string, sectionKey: string, isExpanded: boolean) => {
    console.log(`Shelter settings section ${sectionKey} is now ${isExpanded ? 'expanded' : 'collapsed'}`);
  }, []);
        ...
        <Animated.View style={[styles.overlay, { opacity: overlayOpacity }]}>
          ...
          <Animated.View style={[styles.modalContainer, { transform: [{ translateX }] }]}>
            ...
            <View style={styles.profileSummary}>
              <Text style={styles.profileName}>{shelterData?.shelterName || shelterData?.user?.name || 'Loading...'}</Text>
              <Text style={styles.profileEmail}>{shelterData?.user?.email || ''}</Text>
...
export default ShelterSettingsModal;